...
儘管如此，規劃高權（Planungshoheit）——特別是規劃的具體過程——原則上**仍歸屬於行政部門**：  
...
- 【師】國家不再如早期維持最低限度秩序的「夜警國家」，而扮演更積極的角色
...
「儘管如此，規劃高權，特別是規劃過程，原則上屬於行政機關（包括『優先掌控權』、倡議、起草、決策與執行）。」
...
#### **1.1 主句**
- **主語（Subjekt）：**  
  - *die Planungshoheit*（規劃高權，陰性單數，Nominativ，原形：die Planungshoheit）  
  - *der Planungsprozess*（規劃過程，陽性單數，Nominativ，原形：der Planungsprozess）
- **謂語（Prädikat）：** *liegt*（位於，動詞 *liegen* 的第三人稱單數現在式）
  - *liegen bei der Exekutive*（權限屬於行政機關，介詞 *bei* + Dativ）
- **狀語（Adverbiale）：**  
...
#### **1.3 括號補充內容**
- **「erster Zugriff」**（優先掌控權）【師：首次發動之計劃高權】
  - 【師】斯圖加特火車站的開發案（斯圖加特21計劃），軌道地下化，進行土地大規模徵收，到現在20年還沒好。
- **Initiative**（倡議）
- **Ausarbeitung**（起草）
- **Entscheidung**（決策）
- **Durchführung**（執行）
...
| 變格形式 | 原形（主格） | 性 | 數 | 格 | 解釋 |
|----------|-------------|----|----|----|------|
| *die Planungshoheit* | **die Planungshoheit** | 陰性 | 單數 | 主格 | 規劃高權 |
...
### **3. 法學背景**
#### **3.1 Rn. 347 的核心主題**
- 本句進一步確立**規劃高權（Planungshoheit）**屬於**行政機關（Exekutive）**，特別是在政策制定與執行過程中。  
